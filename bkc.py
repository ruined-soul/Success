...
# Constants
bot_token = '7322708595:AAENv8QB9Vij2DM5Sd0OcWOaJ93PayhUc7I'
developer_id = 1159381624
developer_username = '@ruined_soul'
...
def send_help(message):
    help_text = (
        ...
        "/setspamlimit <number> - Set spam message limit (Admin only)\n"
        "/blockuser <username> - Block user from leaderboard (Admin only)\n"
        "/unblockuser <username> - Unblock user from leaderboard (Admin only)\n"
        "/rank - Show your current rank\n"
        "/leaderboard - Display the top active members\n"
        "/stats - Show your statistics\n"
        "/achievements - List your achievements and badges\n"
        "/dailychallenge - Participate in the daily challenge\n"
        "/weeklychallenge - Participate in the weekly challenge\n"
        "/suggest <message> - Send feedback or suggestions\n"
        "/setmessagetypes <types> - Set message types to count (Admin only)\n"
        "/authorizegroup <group_id> - Authorize a group (Admin only)\n"
    )
...
def block_user(message):
    if message.from_user.id == developer_id:
        try:
            ...
            bot.reply_to(message, "Please provide a username to block.")
